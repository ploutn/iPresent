...
const PresentationEditor: React.FC<PresentationEditorProps> = React.memo(
  ({ presentation, onSave, onCancel }) => {
    ...
    const [fontSize, setFontSize] = useState(
      currentSlide.fontSize?.toString() || "24"
    );
    ...
    const handleSave = useCallback(
      (shouldClose: boolean = false) => {
        ...
        const updatedPresentation: PresentationContentItem = {
          ...presentation,
          slides:
          ...
          settings: {
            ...
            defaultSlideDuration:
              presentation.settings?.defaultSlideDuration || 5,
            theme: presentation.settings?.theme || "church",
            transition: presentation.settings?.transition || "fade",
            loopPresentation: presentation.settings?.loopPresentation || false,
            showSlideNumbers: presentation.settings?.showSlideNumbers ?? true,
            showProgressBar: presentation.settings?.showProgressBar ?? true,
            allowRemoteControl:
              presentation.settings?.allowRemoteControl ?? true,
            backgroundColor:
              presentation.settings?.backgroundColor || "#000000",
            ...
            resolution: presentation.settings?.resolution || {
              width: 1920,
              height: 1080,
            },
          },
          metadata: {
            ...
            version: "1.0",
            totalSlides:
              presentation.slides.length > 0 ? presentation.slides.length : 1,
            estimatedDuration: (presentation.slides.length > 0
              ? presentation.slides
              : [updatedSlide]
            ).reduce(
              (sum, slide) =>
                sum +
                (slide.duration ||
                  presentation.settings?.defaultSlideDuration ||
                  5),
              0
            ),
            isPublic: presentation.metadata?.isPublic || false,
            isTemplate: presentation.metadata?.isTemplate || false,
          },
        };

        onSave(updatedPresentation, shouldClose);
    ...
    useEffect(() => {
      if (autoSave) {
        const timer = setTimeout(() => {
          handleSave(false); // Don't close on auto-save
        }, 1000);
    ...
    const handleAddSlide = () => {
      const newSlide: Slide = {
        ...
        textAlign: "center",
        elements: [],
        duration: presentation.settings?.defaultSlideDuration || 5,
      };
      ...
      const updatedPresentation: PresentationContentItem = {
        ...
        metadata: {
          ...
          estimatedDuration: [...presentation.slides, newSlide].reduce(
            (sum, slide) =>
              sum +
              (slide.duration ||
                presentation.settings?.defaultSlideDuration ||
    ...
    const handleExportPresentation = () => {
      const dataStr =
        ...
        encodeURIComponent(JSON.stringify(presentation));
      const downloadAnchorNode = document.createElement("a");
      downloadAnchorNode.setAttribute("href", dataStr);
      downloadAnchorNode.setAttribute(
        "download",
        `${presentation.title || "presentation"}.json`
      );
    ...
    const handleImportPresentation = (
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          try {
            const importedPresentation: PresentationContentItem = JSON.parse(
    ...
    const handleAddElement = (type: "text" | "image" | "video" | "shape") => {
      const newElement: SlideElement = {
        ...
        src:
          type === "image" || type === "video" ? "placeholder.png" : undefined, // Placeholder for media
        ...
        <div className="flex justify-between items-center p-4 bg-white border-b border-gray-200">
          <div className="flex items-center space-x-4">
            <input
              ...
              onChange={(e) =>
                onSave({ ...presentation, title: e.target.value })
              }
              className="text-2xl font-bold text-gray-800 px-3 py-1 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div className="w-64 bg-white border-r border-gray-200 overflow-y-auto">
            ...
            <div className="p-4 border-t border-gray-200">
              ...
              <div className="space-y-2">
                ...
                <label
                  htmlFor="import-presentation-file"
